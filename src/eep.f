      ...
      ! takes a regular track (read from log) and creates an
      ...
      t% EEP(ieep) = RGBTip(t,t% EEP(ieep-1)); if(t% EEP(ieep)==0) return; ieep=ieep+1
      t% EEP(ieep) = ZAHB(t,t% EEP(ieep-1)); if(t% EEP(ieep)==0) return; ieep=ieep+1
      t% EEP(ieep) = TAHB(t,1d-1,t% EEP(ieep-1)); if(t% EEP(ieep)==0) return; ieep=ieep+1
      t% EEP(ieep) = TAHB(t,1d-4,t% EEP(ieep-1)); if(t% EEP(ieep)==0) return; ieep=ieep+1
      t% EEP(ieep) = TPAGB(t,t% EEP(ieep-1)); if(t% EEP(ieep)==0) return; ieep=ieep+1
      t% EEP(ieep) = PostAGB(t,t% EEP(ieep-1)); if(t% EEP(ieep)==0) return; ieep=ieep+1
